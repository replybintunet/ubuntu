...
function buildFFmpegArgs(
  ...
  if (sourceType === "camera") {
    ...
    if (isNetworkUrl) {
      // Remote camera via RTSP or HTTP stream URL
      args.push("-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "10");
      args.push("-i", inputUrl);
    } else {
      // Local device — detect platform
    ...
    // YouTube — reconnect, no TikTok headers
    if (!isHls) {
      args.push("-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "10");
    }
    args.push("-i", inputUrl);
